...
class OrdersToInvoiceController extends Controller
{
   
    public function readOrdersToInvoice()
    {
        $odoo = new \Edujugon\Laradoo\Odoo();
        $odoo = $odoo->connect();

        $result = $odoo->where('invoice_status','to invoice')
                        /*->limit(8)*/
                        ->get('sale.order');
        return json_encode([
            'result' => $result]);
